...
public class GUIConfiguration extends GuiScreen {
	...
	public GUIConfiguration(EntityPlayer player, World world, int x, int y, int z) {
	...
	public void actionPerformed(GuiButton button) {
		...
		// Map Boundaries
		if (button.id == 2)
			player.openGui(Goldfish.instance, GUIHandler.CONFIG_LIMIT_GUI_ID, player.getEntityWorld(), 0, 0, 0);
		
		// Block Permissions
		if (button.id == 3) {
			
			// Open Block Permissions in Config
		}
		
		// Respawn Options
		if (button.id == 4)
			player.openGui(Goldfish.instance, GUIHandler.CONFIG_RESPAWN_OPTIONS_GUI_ID, player.getEntityWorld(), 0, 0, 0);
		
		// Instance Timers
		if (button.id == 5)
			player.openGui(Goldfish.instance, GUIHandler.CONFIG_TIMERS_GUI_ID, player.getEntityWorld(), 0, 0, 0);
		
		// Equipment Storage
		if (button.id == 6)
			player.openGui(Goldfish.instance, GUIHandler.CONFIG_EQUIPMENT_GUI_ID, player.getEntityWorld(), 0, 0, 0);
		
		// Conditional Entry
		if (button.id == 7)
			player.openGui(Goldfish.instance, GUIHandler.CONFIG_CONDITIONS_GUI_ID, player.getEntityWorld(), 0, 0, 0);
	}
}